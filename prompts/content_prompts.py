...
def generate_outline_prompt(context_data: str | dict, num_slides: int | str) -> tuple[str, str]:
    layout_rules = (
        ...
        "- Ensure that 'requires_image' is set to true for the cover and all floor-specific slides so the app can generate beautiful architectural visuals."
    )
    ...
    if str(num_slides).lower() == "auto":
        system = (
            ...
            "4. IMAGE REQUIREMENT: Set 'requires_image': true for the Cover slide, and for ALL slides describing specific architectural floors, interiors, or exteriors. Write a highly descriptive English 'image_prompt'.\n"
            f"5. {layout_rules}\n"
    ...
    system = (
        ...
        "4. IMAGE REQUIREMENT & SUGGESTED PROMPTS: Set 'requires_image': true for the Cover slide, and for ALL slides that describe specific architectural floors, interiors, or exterior designs. For every slide with 'requires_image': true, you MUST write a highly descriptive English image prompt under the key 'image_prompt' (e.g. 'A photorealistic architectural visualization of...').\n"
        f"5. {layout_rules}\n"
    ...
    if section and structured_data:
        ...
        if section == "cover":
            user_parts.append(
                ...
                f"- نوع المشروع: {structured_data.get('cover_project_type')} {structured_data.get('cover_project_type_custom', '')}\n"
            ...
            user_parts.append(
                f"- نوع المشروع المقترح: {structured_data.get('exec_project_type')}\n"
            ...
            user_parts.append("Format as two_column: {\"title\": \"خصائص الموقع ونظم البناء\", \"bullets\": [\"مخطط رقم ... وقطعة رقم ...\", \"شكل الأرض ... ومناسيبها ...\", \"نظام البناء والارتدادات: ...\", \"البنية التحتية المتوفرة: ...\", \"ملاحظات إضافية وجغرافية: ...\"]}")
            ...
            user_parts.append(
                ...
                f"- البديل الأول: {structured_data.get('dev_opt1_name')} - {structured_data.get('dev_opt1_desc')}\n"
            )
            if structured_data.get('dev_opt2_name'):
                user_parts.append(f"- البديل الثاني: {structured_data.get('dev_opt2_name')} - {structured_data.get('dev_opt2_desc')}\n")
            if structured_data.get('dev_opt3_name'):
                user_parts.append(f"- البديل الثالث: {structured_data.get('dev_opt3_name')} - {structured_data.get('dev_opt3_desc')}\n")
            if structured_data.get('dev_opt4_name'):
                user_parts.append(f"- البديل الرابع: {structured_data.get('dev_opt4_name')} - {structured_data.get('dev_opt4_desc')}\n")
                
            reason_str = structured_data.get('dev_recommendation_reason', '')
            if reason_str == "أخرى":
                reason_str = structured_data.get('dev_recommendation_reason_custom', 'أسباب أخرى')
            ...
            user_parts.append(
                "Format as a chart/table comparison. Make it a detailed table representing the alternatives. "
                "Format: {\"title\": \"بدائل التطوير ودراسة البديل الموصى به\", \"data\": [{\"label\": \"اسم البديل الأول\", \"value\": \"وصف مختصر ومساحة BUA\"}, ...]}"
            )
