...
def _generate_content_inner(system_prompt: str, user_prompt: str, images: list[dict] = None) -> str:
    """Inner retried text generation logic via OpenRouter."""
    logger.info("OpenRouter request (%s): %s...", OPENROUTER_MODEL, user_prompt[:80])
    client = _get_client()
    ...
    try:
        ...
        logger.error("OpenRouter text generation failed: %s", e, exc_info=True)
        raise e


def generate_content(system_prompt: str, user_prompt: str, images: list[dict] = None) -> str:
    """Generate text content using OpenRouter. Fails instantly if key is missing."""
    if not _get_api_key():
        raise ValueError(
            "OpenRouter API Key is missing. "
            "Please set OPENROUTER_API_KEY in your environment, or provide it in the API settings sidebar."
...
def _generate_json_inner(system_prompt: str, user_prompt: str, stream_callback=None, images: list[dict] = None, use_chat_model: bool = False) -> dict[str, Any]:
    """Inner retried JSON generation logic via OpenRouter."""
    target_model = OPENROUTER_CHAT_MODEL if use_chat_model else OPENROUTER_MODEL
    logger.info("OpenRouter JSON request (%s): %s...", target_model, user_prompt[:80])
    client = _get_client()
    ...
    try:
        ...
        logger.info("OpenRouter JSON response: %d chars", len(raw))
        return json.loads(raw)
    except Exception as e:
        logger.error("OpenRouter JSON generation failed: %s", e, exc_info=True)
        ...
        for closure in closures:
            try:
                ...
                continue
        raise ValueError(
            f"OpenRouter did not return valid JSON after auto-repair attempts. "
...
def generate_json(system_prompt: str, user_prompt: str, stream_callback=None, images: list[dict] = None, use_chat_model: bool = False) -> dict[str, Any]:
    """Generate structured JSON content using OpenRouter. Fails instantly if key is missing."""
    if not _get_api_key():
        raise ValueError(
            "OpenRouter API Key is missing. "
            "Please set OPENROUTER_API_KEY in your environment, or provide it in the API settings sidebar."
