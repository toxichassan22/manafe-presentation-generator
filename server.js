...
app.post('/api/generate-titles', async function(req, res) {
  var projectData = truncateProjectData(req.body.projectData, 4000);
  var userId = req.body.userId || 'default_user';
  var totalSlides = parseInt(req.body.slideCount) || 12;
  ...
  try {
    ...
    console.log('  ✓ Got ' + titles.length + ' content titles for ' + totalSlides + ' total slides | Cache: ' + cacheAnalytics.status);
    res.json({ success: true, titles: titles, totalSlides: totalSlides, cache_analytics: cacheAnalytics });
  } catch (err) {
